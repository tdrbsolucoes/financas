...
const MapPicker = ({ lat, lng, onLocationChange, address }) => {
    ...
    useEffect(() => {
        if (mapRef.current && markerRef.current && typeof L !== 'undefined') {
...
interface NewContactsPageProps {
  user: User
  onDatabaseError?: (error: any) => void
}

const NewContactsPage: React.FC<NewContactsPageProps> = ({ user, onDatabaseError }) => {
